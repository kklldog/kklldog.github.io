...
### Otlp
总的来说 OpenTelemetry 最大的有点是他的 “标准化”。在数字的世界里，标准才是王道，比如 TCP, Http。OpenTelemetry 定义了 Otlp（OpenTelemetry Protocol） 协议。通过 Otlp 对接其他组件。因为大家都知道市面上做观测的产品有很多，有 Elastic APM 这种全家桶，有专注于 Traceing 的 Jaeger，有专注日志的 loki 跟 Seq。   
...
## Seq
Seq 是一款使用现代化技术构建的结构化日志存储，查询，分析工具。比起 ELK 这种组合要轻量级许多。只需要一个安装包就具有数据存储，查询，图表分析功能。它对 windows 友好，直接提供了安装包。当然也可以使用 docker 来部署。Seq 对于单个用户是免费的，比较遗憾的是并不开源。而且 Seq 本身使用.NET实现。Seq 一个比较强大的功能是提供了类似 Sql 语句的数据查询及处理能力，使得用户可以直接写 Select from 来得到自己想要的数据。  
...
## 总结
OpenTelemetry 提供了一种简单的方式来在 .NET 应用程序中记录日志。通过使用 OpenTelemetry，你可以轻松地收集和分析你的应用程序的遥测数据，从而更好地理解和优化你的应用程序的性能和行为。OpenTelemetry 提供了一套通用的传输协议 Otlp。可以使我们做设计跟开发产品或项目的时候轻松跟第三方可观测组件进行解耦。
